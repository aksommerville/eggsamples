...
static void field_render_cellv_rm(struct field *field,int16_t dstx,int16_t dsty) {
  ...
  int yi=FIELDH; for (;yi-->0;rmv--,dsty-=NS_sys_tilesize) {
    if (*rmv) {
      graf_fill_rect(&g.graf,dstx-(NS_sys_tilesize>>1),dsty-(NS_sys_tilesize>>1),FIELDW*NS_sys_tilesize,NS_sys_tilesize,hilite);
    } else {
      int xi=FIELDW;
      ...
      for (;xi-->0;dstx1+=NS_sys_tilesize,p++) {
        if (!*p) continue;
        graf_tile(&g.graf,dstx1,dsty,*p,0);
      }
      rowv-=FIELDW;
    }
  }
  ...
  if (field->rmclock>0.0) {
    ...
    int yi=FIELDH; for (;yi-->0;dsty+=NS_sys_tilesize) {
      int xi=FIELDW; for (dstx=dstx0;xi-->0;dstx+=NS_sys_tilesize,v++) {
        ...
        graf_tile(&g.graf,dstx,dsty,*v,0);
      }
    }
  }
}
...
static void field_render_player(struct field *field,const struct player *player,int16_t dstx,int16_t dsty) {
  ...
  for (;tilec-->0;tile++) {
    ...
    graf_tile(&g.graf,dstx+col*NS_sys_tilesize,dsty+row*NS_sys_tilesize,tileid,0);
  }
}
...
static void field_render_level(struct field *field,int16_t parentx,int16_t parenty) {
  ...
  if (field->disp_level!=field->level) {
    ...
    if ((tmpc>0)&&(tmpc<sizeof(tmp))) field->level_texid=font_render_to_texture(0,g.font,tmp,tmpc,parentw,font_get_line_height(g.font),0xffffffff);
    egg_texture_get_size(&field->level_w,&field->level_h,field->level_texid);
  }
  ...
  graf_set_input(&g.graf,field->level_texid);
  graf_decal(&g.graf,dstx,dsty,0,0,field->level_w,field->level_h);
...
void field_render(struct field *field,int16_t dstx,int16_t dsty) {
  ...
  if (field->finished) {
    graf_fill_rect(&g.graf,dstx,dsty,FIELDW*NS_sys_tilesize,FIELDH*NS_sys_tilesize,0x400010ff);
  }
...
void field_render_next(struct field *field,int16_t dstx,int16_t dsty,int16_t dstw,int16_t dsth) {
  ...
  if ((dstw>=NS_sys_tilesize<<2)&&(dsth>=NS_sys_tilesize<<2)) {
    ...
    for (i=tilec;i-->0;tile++) {
      graf_tile(&g.graf,figx+(tile->x-xa)*NS_sys_tilesize,figy+(tile->y-ya)*NS_sys_tilesize,tileid,0);
    }
    
  // If too small, use the single-tile thumbnails in row 1.
  } else {
    graf_tile(&g.graf,dstx+(dstw>>1),dsty+(dsth>>1),0x11+tetr,0);
  }
}
...
void field_render_single_score(struct field *field,int16_t dstx,int16_t dsty,int16_t dstw,int16_t dsth) {
  ...
  if (field->disp_score!=field->score) {
    ...
    field->score_texid=font_render_to_texture(0,g.font,tmp,tmpc,dstw,dsth,0xffffffff);
    egg_texture_get_size(&field->score_w,&field->score_h,field->score_texid);
    field->disp_score=field->score;
  }
  
  graf_set_input(&g.graf,field->score_texid);
  graf_decal(&g.graf,dstx,dsty+(dsth>>1)-(field->score_h>>1),0,0,field->score_w,field->score_h);
}
...
void field_render_combined_score(struct field *l,struct field *r,int16_t dstx,int16_t dsty,int16_t dstw,int16_t dsth) {
  ...
  if ((l->disp_score!=l->score)||(r->disp_score!=r->score)) {
    ...
    if (((tmpc=snprintf(tmp,sizeof(tmp),"%d",r->score))>0)&&(tmpc<sizeof(tmp))) {
      int sw=font_measure_string(g.font,tmp,tmpc);
  ...
  graf_set_input(&g.graf,l->score_texid);
  graf_decal(&g.graf,dstx+(dstw>>1)-(l->score_w>>1),dsty+(dsth>>1)-(l->score_h>>1),0,0,l->score_w,l->score_h);
}
