...
void textbox_render_backdrop(int16_t x,int16_t y,int16_t w,int16_t h) {
  if ((w<BORDERW*2)||(h<BORDERW*2)) {
    ...
    graf_fill_rect(&g.graf,x+BORDERW,y+BORDERW,w-BORDERW*2,h-BORDERW*2,0x000000ff);
  }
}
...
void textbox_render_border(int16_t x,int16_t y,int16_t w,int16_t h) {
  ...
  if ((w<NS_sys_tilesize)||(h<NS_sys_tilesize)) {
    graf_fill_rect(&g.graf,x,y,w,1,BORDERCOLOR);
    graf_fill_rect(&g.graf,x,y,1,h,BORDERCOLOR);
    graf_fill_rect(&g.graf,x+w-1,y,1,h,BORDERCOLOR);
    graf_fill_rect(&g.graf,x,y+h-1,w,1,BORDERCOLOR);
    return;
  }
  ...
  if (w<=NS_sys_tilesize+BORDERW) {
    ...
    graf_tile(&g.graf,x+(w>>1),y+(BORDERW>>1),0x11,0);
    graf_tile(&g.graf,x+(w>>1),y+h-(BORDERW>>1),0x11,EGG_XFORM_YREV);
  } else {
    graf_tile(&g.graf,x+w-NS_sys_tilesize,y+(BORDERW>>1),0x11,0);
    graf_tile(&g.graf,x+w-NS_sys_tilesize,y+h-(BORDERW>>1),0x11,EGG_XFORM_YREV);
  }
  if (h<=NS_sys_tilesize+BORDERW) {
  } else if (h<=NS_sys_tilesize*2+BORDERW) {
    graf_tile(&g.graf,x+w-(BORDERW>>1),y+(h>>1),0x11,EGG_XFORM_SWAP|EGG_XFORM_YREV);
    graf_tile(&g.graf,x+(BORDERW>>1),y+(h>>1),0x11,EGG_XFORM_SWAP);
  } else {
    graf_tile(&g.graf,x+w-(BORDERW>>1),y+h-NS_sys_tilesize,0x11,EGG_XFORM_SWAP|EGG_XFORM_YREV);
    graf_tile(&g.graf,x+(BORDERW>>1),y+h-NS_sys_tilesize,0x11,EGG_XFORM_SWAP);
  }
  
  /* A tile at each corner.
   */
  graf_tile(&g.graf,x+(BORDERW>>1),y+(BORDERW>>1),0x10,0);
  graf_tile(&g.graf,x+w-(BORDERW>>1),y+(BORDERW>>1),0x10,EGG_XFORM_SWAP|EGG_XFORM_YREV);
  graf_tile(&g.graf,x+w-(BORDERW>>1),y+h-(BORDERW>>1),0x10,EGG_XFORM_XREV|EGG_XFORM_YREV);
  graf_tile(&g.graf,x+(BORDERW>>1),y+h-(BORDERW>>1),0x10,EGG_XFORM_SWAP|EGG_XFORM_XREV);
  ...
  for (;i-->0;dst+=NS_sys_tilesize) {
    graf_tile(&g.graf,dst,y+(BORDERW>>1),0x11,0);
    graf_tile(&g.graf,dst,y+h-(BORDERW>>1),0x11,EGG_XFORM_YREV);
  }
  for (dst=y+NS_sys_tilesize,i=(h-NS_sys_tilesize)/NS_sys_tilesize;i-->0;dst+=NS_sys_tilesize) {
    graf_tile(&g.graf,x+(BORDERW>>1),dst,0x11,EGG_XFORM_SWAP);
    graf_tile(&g.graf,x+w-(BORDERW>>1),dst,0x11,EGG_XFORM_SWAP|EGG_XFORM_YREV);
  }
}
...
void textbox_render(struct textbox *textbox) {
  ...
  if (textbox->focus==2) {
    if (textbox->selp>=0) {
      ...
      graf_fill_rect(&g.graf,dstx,dsty,dstw,dsth,color);
    }
  }
...
void textbox_render_more(int16_t x,int16_t y,int16_t w,int16_t h,uint8_t mask) {
  ...
  if (g.framec%50>=30) tileid++;
  graf_set_image(&g.graf,RID_image_uitiles);
  if (mask&DIR_N) graf_tile(&g.graf,x+(w>>1),y+1,tileid,EGG_XFORM_YREV);
  if (mask&DIR_S) graf_tile(&g.graf,x+(w>>1),y+h-1,tileid,0);
  if (mask&DIR_W) graf_tile(&g.graf,x+1,y+(h>>1),tileid,EGG_XFORM_SWAP|EGG_XFORM_YREV);
  if (mask&DIR_E) graf_tile(&g.graf,x+w-1,y+(h>>1),tileid,EGG_XFORM_SWAP);
}
